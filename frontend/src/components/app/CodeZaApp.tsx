...
<body>
  ...
  <\/script>
    ...
    const safeError = (error || "").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&#39;");
     ...
     let bodyContent = '';
     if (!code) {
       bodyContent = '<div class="empty-state"><p>?? Start typing code to see results here</p></div>';
     } else if (isRunning) {
       ...
       let error_html = '';
       if (safeError) {
         error_html = `<div class="error-section"><span class="error-label">?? Error:</span>${safeError}</div>`;
       }
       let empty_html = '';
       ...
       let duration_html = '';
       if (duration !== null) {
         duration_html = `<div class="duration">?? Execution time: ${duration}ms</div>`;
       }
        ...
        <main className="flex-1 flex flex-col relative z-10">
          ...
          <section className={`py-32 px-12 grid grid-cols-1 lg:grid-cols-2 gap-20 border-b ${theme === 'dark' ? 'border-white/10' : 'border-black/10'}`}>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className={`aspect-square rounded-3xl ${theme === 'dark' ? 'bg-white/5 border-white/10' : 'bg-black/5 border-black/10'} border flex flex-col items-center justify-center p-8 text-center gap-4`}>
                <span className="text-5xl font-bold tracking-tighter">8</span>
